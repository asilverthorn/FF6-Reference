commands = ["B0", # Set background palette color addition (absolute)
...
searchfile = open("D:/Downloads/battle_anim_script.txt")
ctr = 0
comment = ""
...
for line in searchfile:
    ...
    if line[0] == ';': # found a comment
        ...
        nameStart = comment.rfind(": ")
        nameEnd = comment.rfind(" (")
        name = comment[nameStart+2:nameEnd]
    ...
    if(idx == 7):
        ...
        if(comment != ""):
            ...
            print("\"" + name + "\": [")
            comment = ""
        print("    0x10" + line[3:7] + ", #" + re.sub('\s\s+', ' - ', line[8:len(line)-1]))
        ctr = ctr + 1    
        
print("Total found: " + str(ctr))
